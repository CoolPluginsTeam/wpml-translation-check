...
class CPFM_Feedback_Notice {
    ...
    public function cpfm_enqueue_assets() {
        ...
        wp_enqueue_style('cpfm-common-review-style', AUTOMLP_AI_PLUGIN_URL . 'admin/cpfm-feedback/css/cpfm-admin-feedback.css');
        wp_enqueue_script(
            'cpfm-common-review-script', 
            AUTOMLP_AI_PLUGIN_URL . 'admin/cpfm-feedback/js/cpfm-admin-feedback.js', 
            ['jquery'], 
            AUTOMLP_AI_VERSION, 
            true
    ...
    public function cpfm_render_notice_panel() {
        ...
        $output .= '<div class="notice-panel-header">' . esc_html__('Help Improve Plugins', 'wpml-translation-check') . ' <span class="dashicons dashicons-no" id="cpfm_remove_notice"></span></div>';
        ...
        foreach (self::$registered_notices as $key => $notice) {
            ...
            $output .= '<p>' . esc_html($notice['message']) . '<a href="#" class="cpf-toggle-extra">' . esc_html__(' More info', 'wpml-translation-check') . '</a></p>';
            $output .= '</div>';
            ...
            $output .= '<li>' . esc_html__('To check plugin compatibility, we will collect the following: list of active plugins and themes, server type, MySQL version, WordPress version, memory limit, site language and database prefix.', 'wpml-translation-check') . '</li>';
            ...
            $output .= '<button class="button button-primary opt-in-yes" data-category="' . esc_attr($key) . '" id="yes-share-data" value="yes">' . esc_html__("Yes, I Agree", 'wpml-translation-check') . '</button>';
            $output .= '<button class="button opt-in-no" data-category="' . esc_attr($key) . '" id="no-share-data" value="no">' . esc_html__('No, Thanks', 'wpml-translation-check') . '</button>';
            $output .= '</div>';
