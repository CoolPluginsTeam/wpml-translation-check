<?php
namespace automlp\feedback;

if ( ! defined( 'ABSPATH' ) ) {
	exit; }

class automlp_feedback {

	private $plugin_url     = AUTOMLP_AI_PLUGIN_URL;
	private $plugin_version = AUTOMLP_AI_VERSION;
	private $plugin_name    = 'AutoMLP – AI Translation for WPML';
	private $plugin_slug    = 'automlp';
	...
	function enqueue_feedback_scripts() {
		...
		if ( isset( $screen ) && $screen->id == 'plugins' ) {
			wp_enqueue_script(
				...
				$this->plugin_url . 'admin/feedback/js/admin-feedback.js',
				array( 'jquery' ),
				$this->plugin_version,
				true
			);
	...
	*/
	public function show_deactivate_feedback_popup() {
		...
		$deactivate_reasons = array(
			'didnt_work_as_expected'         => array(
				'title'             => __( 'The plugin didn\'t work as expected.', 'loop-grid-extender-for-elementor-pro' ),
				'input_placeholder' => 'What did you expect?',
			),
			'found_a_better_plugin'          => array(
				'title'             => __( 'I found a better plugin.', 'loop-grid-extender-for-elementor-pro' ),
				'input_placeholder' => __( 'Please share which plugin.', 'loop-grid-extender-for-elementor-pro' ),
			),
			'couldnt_get_the_plugin_to_work' => array(
				'title'             => __( 'The plugin is not working.', 'loop-grid-extender-for-elementor-pro' ),
				'input_placeholder' => 'Please share your issue. So we can fix that for other users.',
			),
			'temporary_deactivation'         => array(
				'title'             => __( 'It\'s a temporary deactivation.', 'loop-grid-extender-for-elementor-pro' ),
				'input_placeholder' => '',
			),
			'other'                          => array(
				'title'             => __( 'Other reason.', 'loop-grid-extender-for-elementor-pro' ),
				'input_placeholder' => __( 'Please share the reason.', 'loop-grid-extender-for-elementor-pro' ),
			),
		);
		...
		<div id="cool-plugins-feedback-<?php echo esc_attr( $this->plugin_slug ); ?>" class="hide-feedback-popup">
			...
			<div class="cp-feedback-header">
				<div class="cp-feedback-title"><?php echo esc_html__( 'Quick Feedback', 'loop-grid-extender-for-elementor-pro' ); ?></div>
			...
			<div class="cp-feedback-form-wrapper">
				<div class="cp-feedback-form-title"><?php echo esc_html__( 'If you have a moment, please share the reason for deactivating this plugin.', 'loop-grid-extender-for-elementor-pro' ); ?></div>
				<form class="cp-feedback-form" method="post">
					...
					<input class="cp-feedback-terms-input" id="cp-feedback-terms-input" type="checkbox"><label for="cp-feedback-terms-input"><?php echo esc_html__( 'I agree to share my feedback with Cool Plugins, including site URL and admin email, to enable them to address my inquiry.', 'loop-grid-extender-for-elementor-pro' ); ?></label>
					</div>
	...
	function automlp_get_user_info() {
		global $wpdb;
	...
	function submit_deactivation_response() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), '_cool-plugins_deactivate_feedback_nonce' ) ) {
			...
			$deactivate_reasons = array(
				...
				'other'                          => array(
				'title'             => __( 'Other', 'loop-grid-extender-for-elementor-pro' ),
					'input_placeholder' => __( 'Please share the reason.', 'loop-grid-extender-for-elementor-pro' ),
				),
			);
			...
			$feedback_url      = AUTOMLP_AI_FEEDBACK_API .'wp-json/coolplugins-feedback/v1/feedback';
